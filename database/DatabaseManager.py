...
class DatabaseManager:
        ...
        cred = credentials.Certificate(
            {
                ...
                "auth_provider_x509_cert_url": st.secrets["default"][
    ...
    def load_skater_data(self, skater_id: int) -> list[TrainingData]:
        ...
        for training in (
            ...
            .where(filter=firestore.firestore.FieldFilter("skater_id", "==", skater_id))
            .order_by("training_date")
            .stream()
        ):
            data_trainings.append(
                TrainingData(
                    training.id,
    ...
    def load_training_data(self, training_id: int) -> list[JumpData]:
        data_jumps = []
        for jump in (
            self.db.collection("jumps")
            .where(
                filter=firestore.firestore.FieldFilter("training_id", "==", training_id)
            )
            .order_by("jump_time")
            .stream()
        ):
            data_jumps.append(
                JumpData(
                    jump.id,
                    jump.get("training_id"),
                    jump.get("jump_type"),
                    jump.get("jump_rotations"),
                    jump.get("jump_success"),
                    jump.get("jump_time"),
                )
            )
        return data_jumps
    ...
    def get_skater_from_training(self, training_id: str) -> int:
        skater_id = (
            self.db.collection("trainings").document(training_id).get().get("skater_id")
        )
    ...
    def get_skater_id_from_name(self, skater_name: str) -> str:
        skater_id = (
            self.db.collection("skaters")
            .where(
                filter=firestore.firestore.FieldFilter("skater_name", "==", skater_name)
            )
            .get()
        )
        return skater_id
    ...
    def find_training(self, date, device_id):
        trainings = (
            self.db.collection("trainings")
            .where(filter=firestore.firestore.FieldFilter("training_date", "==", date))
            .where(filter=firestore.firestore.FieldFilter("dot_id", "==", device_id))
            .get()
        )
        return trainings

    def set_training_date(self, training_id, date) -> None:
        self.db.collection("trainings").document(training_id).update(
            {"training_date": date}
        )
    ...
    def get_current_record(self, device_id) -> str:
        return (
            self.db.collection("dots").document(device_id).get().get("current_record")
        )
    ...
    def get_all_trainings_for_skater(self, skater_id) -> List[TrainingData]:
        trainings = (
            self.db.collection("trainings").where("skater_id", "==", skater_id).stream()
        )
        return [
            TrainingData(
                training.id,
